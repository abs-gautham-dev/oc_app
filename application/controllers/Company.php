...
class Company extends CI_Controller {
	...
	public function order_list()
	{
		...
		$data['breadcrumbs'][] = array(
			...
			'class'=>'',
			'title' => 'Dashboard',
			'link' => site_url('company/dashboard')
		);
		...
		$user_id = $this->session->userdata('user_id');
		$user_id = 1;
		$data['sales_order'] = $this->Common_model->getRecords('sales_order','*',array('sender_id'=>$user_id),'id Desc',false);
		...
		$this->load->view('frontend/company/received_order_list');
		$this->load->view('frontend/include/footer');
	}
	public function send_job_request($id='')
	{
		// echo $id;die;
		$data['title']="Send Job Request | ".SITE_TITLE;
		$data['page_title']="Send Job Request";
		$data['breadcrumbs'] = array();
		$data['breadcrumbs'][] = array(
			'icon'=>'<i class="fa fa-dashboard"></i>',
			'class'=>'',
		...
		$user_id = $this->session->userdata('user_id');
		$user_id =1;
		$data['sales_order'] = $this->Common_model->getRecords('sales_order','*',array('id'=>$id),'id Desc',true);
		$data['user_info'] = $this->Common_model->getRecords('users','*',array('user_id'=>$user_id),'user_id Desc',true);
		

		if($this->input->post()){
			$contact_name = $this->input->post('contact_name');
			$number_of_job = $this->input->post('number_of_job');
			...
			if(empty($record_id)){
				$update_data['created'] = date("Y-m-d H:i:s");
				 if(!$order_id = $this->Common_model->addEditRecords('sales_order', $update_data)) {
			    	$err = array('status' => '0', 'msg' => '<div class="alert alert-warning"><strong>Error!</strong> Some error occured! Please try again.</div>');
				} else {
					$suc = array('status' => '1', 'msg' => '<div class="alert alert-success"><strong>Success !</strong> Job Request Send Successfully.</div>');
		            echo json_encode($suc); exit;
				} 
			}else{

				 if(!$order_id = $this->Common_model->addEditRecords('sales_order', $update_data,array('id'=>$record_id))) {
		...
		$this->load->view('frontend/include/header',$data);
		$this->load->view('frontend/include/breadcrumb');
		$this->load->view('frontend/company/send_job_request');
		$this->load->view('frontend/include/footer');
	}
	...
	public function invoice_list()
	{
		...
		$data['title']="Invoice List | ".SITE_TITLE;
