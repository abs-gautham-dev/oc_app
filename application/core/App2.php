...
class App2 extends CI_Controller {
		    	...
		    	if($res['is_verified']==0){
		    		...
		    		if(!empty($code)){
		    			...
		    			if($this->Common_model->addEditRecords('users',array('token'=>$code),array('user_id'=>$res['user_id']))) {
		    				$err = array('data'=> array('status'=>'2','msg'=>'Verification code has been sent to your phone number.','code'=>$code)) ;
							echo json_encode($err);
							exit;
		    			}
		    		}
	...
	public function signup()
	{
     ...
     //  echo "<pre>";print_r($_FILES);exit;
        ...
        $code = $this->sendotp($mobile);  // Need uncomment
        $mobile_auth_token = base64_encode(rand());
	    $insert_data = array( 
	    	'user_type'=>$user_type,
	    	'full_name'=>$full_name,
		...
		if($user_id = $this->Common_model->addEditRecords('users',$insert_data)) {
			...
			//Send mail 
		 	$this->Common_model->sendEmail($to_email,$subject,$body,$from_email); 
				...
				// }elseif ($this->language=='arabic') {
				// 	$response = array('data'=> array('status'=>'1','msg'=>'تم التسجيل بنجاح  ','details'=>$user_data));

				// }elseif ($this->language=='french') {
				// 	$response = array('data'=> array('status'=>'1','msg'=>'inscription réussie','details'=>$user_data));
				...
				if($this->language=='english'){
					$response = array('data'=> array('status'=>'2','msg'=>'Signup Successfully','details'=>$user_data,'code'=>$code));

				}elseif ($this->language=='arabic') {
					$response = array('data'=> array('status'=>'2','msg'=>'تم التسجيل بنجاح  ','details'=>$user_data,'code'=>$code));

				}elseif ($this->language=='french') {
					$response = array('data'=> array('status'=>'2','msg'=>'inscription réussie','details'=>$user_data,'code'=>$code));
