...

{
...

	{
		...
		$headers = 'From: '.WEBSITE_NAME.' '.INFO_EMAIL . "\r\n" ;
...

    {
        ...
        $this->db->select('u.payment_status,u.full_name,u.profile_pic,u.email,u.mobile,u.about,c.name as category_name,con.name as country_name,states.name as state_name,cities.name as city_name');
 ...
 public function getRoomidCheck($sender_id,$receiver_id,$type='')

    {
       ...
       $where = '((sender_id='.$sender_id.' and receiver_id='.$receiver_id.' ) OR (sender_id='.$receiver_id.' and receiver_id='.$sender_id.' )) AND type="'.$type.'" ';
...

        {
           	...
           	$this->db->select('max(room_id) as room_id');
 ...
 public function getRoomidid($sender_id,$receiver_id,$type='')
    ...
    public function checkblock($sender_id,$receiver_id)

    {
        ...
        $where = '((sender_id='.$sender_id.' and receiver_id='.$receiver_id.' ) OR (sender_id='.$receiver_id.' and receiver_id='.$sender_id.' )) AND is_blocked="1" ';
...
public function getChatlisting($sender_id,$receiver_id)
{
        ...
        $where = '((sender_id='.$sender_id.' and receiver_id='.$receiver_id.' ) OR (sender_id='.$receiver_id.' and receiver_id='.$sender_id.' )) AND `type`="chat" and `message`!="" ';

       	$this->db->where($where);
  		$this->db->order_by('users_chat.id','ASC'); 
        $query = $this->db->get();
    ...
    public function checkAlreadyCall($receiver_id,$type)
    {
        $this->db->select('on_call');
        $this->db->from('users_chat');

        $where = '(sender_id='.$receiver_id.' OR receiver_id='.$receiver_id.' )  AND type="'.$type.'" AND on_call="1" ';
       	$this->db->where($where);
        $query1 = $this->db->get();
        $record1 = $query1->row_array();
        if(!empty($record1))
    ...
    public function getchatlist($user_id)
    {
				...
				$query = $this->db->query('SELECT users_chat.*
				FROM `users_chat`
				INNER JOIN
				(  SELECT max(id) as id
				    FROM users_chat
				  ...
				  ON users_chat.id = p2.id
				WHERE (`sender_id` =  '.$user_id.' or `receiver_id` = '.$user_id.' )
				ORDER BY users_chat.id DESC');
    ...
    function push_notification_send($device_id,$device_type,$alert_message,$notification_type='notification') {  
        $iosarray = array(
            'alert' => $alert_message,
    ...
    function simple_notification_list(){
    	$this->db->select('group_concat(u.full_name) as notification_sent,a.username as send_by,n.notification_title,n.created');

        $this->db->from('notifications n');

        $this->db->join('users u','u.user_id = n.user_id');

        $this->db->join('admin a','a.admin_id = n.created_by');

        $this->db->where('n.type','simple_notification');
        $this->db->where('n.is_admin','1');

        $this->db->group_by('n.group_by'); 
    ...
    function advertisementList(){
    	$this->db->select('group_concat(u.full_name) as notification_sent,a.username as send_by,n.notification_title,n.created,n.image,n.notification_description');

        $this->db->from('advertisement n');

        $this->db->join('users u','u.user_id = n.user_id');

        $this->db->join('admin a','a.admin_id = n.created_by');

        $this->db->where('n.type','ad_notification');
        $this->db->where('n.is_admin','1');
    ...
    function appointmentList($dr_id){
    	$this->db->select('a.created,u.full_name,u.full_name,u.email,u.profile_pic,u.mobile');
    ...
    function feedbackList($dr_id){
    	$this->db->select('a.id,a.created,,u.full_name,u.full_name,u.email,u.profile_pic,u.mobile');
    ...
    function individual_feedback($dr_id){
    	$this->db->select('a.id,a.created,,u.full_name,u.full_name,u.email,u.profile_pic,u.mobile,u.user_id,a.random');
        $this->db->from('individual_feedback_answer a');
        $this->db->join('users u','a.user_id = u.user_id');
		$this->db->where('a.doctor_id',$dr_id); 
        $this->db->order_by('a.id','Desc'); 
        $this->db->group_by('a.random'); 
    ...
    function individual_feedback_view($random){
    	$this->db->select('*');
        $this->db->from('individual_feedback');
        $this->db->join('individual_feedback_answer','individual_feedback_answer.question_id = individual_feedback.id');
		$this->db->where('individual_feedback_answer.random',$random); 
  ...
  //       	$this->db->like('ads.title',$name);
   ...
   public function getGoogleRating($page_id) { 
	...
	public function google_busniess_list($keyword,$lat,$long,$places='')
 	...
 	#gautam
 	public function select_column($table_name,$select,$where='',$group_by='',$where_or=''){
